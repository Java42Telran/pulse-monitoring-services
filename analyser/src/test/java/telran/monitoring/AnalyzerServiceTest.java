...
class AnalyzerServiceTest {
	...
	void noRedisDataTest() {
		assertNull(service.processProbe(new PulseProbe(PATIENT_NO_REDIS_DATA, VALUE)));
	}
	@Test
	void  noJumpTest() {
		assertNull(service.processProbe(new PulseProbe(PATIENT_NO_JUMP, VALUE)));
	}
	@Test
	void  jumpTest() {
		assertEquals(this.jump, service.processProbe(new PulseProbe(PATIENT_JUMP, JUMP_VALUE)));
	}
