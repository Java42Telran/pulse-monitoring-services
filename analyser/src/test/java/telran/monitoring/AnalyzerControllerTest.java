...
public class AnalyzerControllerTest {
	...
	PulseProbe probeNoJump = new PulseProbe(PATIENT_ID_NO_JUMP, VALUE);
	PulseProbe probeJump = new PulseProbe(PATIENT_ID_JUMP, VALUE);
	...
	@BeforeEach
	void mockingService() {
		when(service.processProbe(probeJump)).thenReturn(jump);
		when(service.processProbe(probeNoJump)).thenReturn(null);
	}
@Test
